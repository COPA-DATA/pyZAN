...
# This is the start timestamp of available data in UTC. The sample requires 3 hours and 40
# minutes of data starting at this timestamp.
...
def sliceData(x, lag):
    ...
    while i < len(x.index) - lag:
        ...
        if x.iloc[i]['R1_Status'] == 0 and x.iloc[i - 1]['R1_Status'] != 0:
            ...
            slices.append(sl.assign(nr = count))
            count = count + 1
            i = i + lag - flex
        else:
            i = i + 1
...
# Get a mean slice ...
healthy_mean_slice = healthy_sliced.groupby(level = 1).mean()
fig, ax = plt.subplots()
...
unhealthy = zan.read_Online_Archive(project_reference = "PREDICTIVE_MAINTENANCE_DEMO_820",\
                                    ...
                                    "RobotSimForPA/Global/R1_Status",\
                                    "RobotSimForPA/Global/R1_WeldingVoltage"],\
                                    time_from = reference_time + datetime.timedelta(hours = 1, minutes = 20),\
                                    time_to = reference_time + datetime.timedelta(hours = 1, minutes = 50),\
                                    show_visualnames = True)
...
unhealthy_mean_slice = unhealthy_sliced.groupby(level = 1).mean()
fig, ax = plt.subplots()
...
ml_data = pd.concat([healthy_ml.assign(label = 'healthy'), unhealthy_ml.assign(label = 'unhealthy')])
...
test_data = zan.read_Online_Archive(project_reference = "PREDICTIVE_MAINTENANCE_DEMO_820",\
                                    ...
                                    "RobotSimForPA/Global/R1_Status",\
                                    "RobotSimForPA/Global/R1_WeldingVoltage"],\
                                    time_from = reference_time + datetime.timedelta(hours = 2, minutes = 40),\
                                    time_to = reference_time + datetime.timedelta(hours = 3, minutes = 40),\
                                    show_visualnames = True)
...
labels = labeled_data_sliced.groupby(level = 0).mean().R1_Error3_Toggle.apply(lambda x: 'healthy' if x < 0.5 else 'unhealthy')
